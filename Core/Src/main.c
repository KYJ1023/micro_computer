...
void StartDefaultTask(void const * argument)
{
  ...
  for(;;)
  {
    osDelay(1);								// freeRTOS에서는 HAL_Delay 사용불가	// 디폴트로 osDelay(1)을 해줘야 함
    HAL_GPIO_TogglePin(GPIOA,GPIO_PIN_11);	// Green LED 토글 //0.5초 마다 PA11번 핀
    osDelay(500);
  }
  /* USER CODE END 5 */
}
...
void StartTask02(void const * argument)
{
  ...
  {
    osDelay(1);
    HAL_GPIO_TogglePin(GPIOB,GPIO_PIN_11);	// Yellow LED 토글 //1초 마다 PB11번 핀
    osDelay(1000);
  }
...
void StartTask03(void const * argument)
{
  ...
  {
    osDelay(1);
    HAL_GPIO_TogglePin(GPIOB,GPIO_PIN_12);	// Red LED 토글 //1.5초 마다 PB12번 핀
    osDelay(1500);
  }
